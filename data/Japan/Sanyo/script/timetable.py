...
# -*- coding: utf-8 -*-
"""
timetable.py — 山陽電気鉄道 navitime 爬蟲（本線 + 網干線）

平日/土休の 2 ファイルを出力。navitime の timetable listing は「サーバ当日から約 4 日」の
プールを返し URL の date は無視される。各 stopCode の data-date を**集合**で蓄積し、
代表日（平日/土休）がその集合に含まれるかで分桶（一部の毎日運行 code は複数日に再利用
されるため、first-date 去重だと片方の桶が激減する＝Chizu で踩過坑）。

種別は data-name。直通特急等は西代以東の神戸高速→阪神（大阪梅田方面）へ直通 → 拓樸外駅は
stops に残し convert で is_other に振り分ける。方向は stops 序列で判断。
...
# 山陽電鉄 2 路線（navitime companyId: 00000066）
LINE_MAIN   = "00000675"  # 本線
LINE_ABOSHI = "00000676"  # 網干線
SCAN_LINES = [LINE_MAIN, LINE_ABOSHI]
...
def scan_station(node, scan_line):
    """{stopCode: (種別, code_lineId, ref_node, set(運行日))}。日付は集合で蓄積。"""
    found = {}
    for updown in [0, 1]:
...
def _pick_representative(all_codes, want_weekend):
    ...
    bucket = {c: (all_codes[c][0], all_codes[c][1], all_codes[c][2], rep_date) for c in codes}
...
def run(max_workers=3, only=None):
    ...
    print("🔍 [第一段階] 全駅の時刻表を掃描（4日プール全件、data-date 集合）…", flush=True)
    all_codes = {}
