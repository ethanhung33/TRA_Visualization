...
# -*- coding: utf-8 -*-
"""
timetable.py — 阪神電気鉄道 navitime 爬蟲（本線 + なんば線 + 武庫川線）

平日/土休の 2 ファイルを出力。navitime の timetable listing は「サーバ当日から約 4 日」の
プールを返し URL の date は無視されるため、各班 <li.time-frame> の data-date の曜日で
平日/土休に分桶し、各カテゴリの「最多班次の代表日」を採用（水〜日に実行すれば両方含む）。

種別は data-name。方向は stops 序列で判断。直通先（近鉄・山陽・神戸高速）の駅は
拓樸外 → そのまま stops に残し、convert 側で is_other segment に振り分ける。
...
# 阪神 3 路線（navitime companyId: 00000058）
LINE_MAIN     = "00000663"  # 本線
LINE_NAMBA    = "00000661"  # なんば線
LINE_MUKOGAWA = "00000662"  # 武庫川線
SCAN_LINES = [LINE_MAIN, LINE_NAMBA, LINE_MUKOGAWA]
...
def scan_station(node, scan_line):
    """{stopCode: (種別, code_lineId, ref_node, data_date)}"""
    found = {}
    for updown in [0, 1]:
        ...
        for li in soup.select("li.time-frame"):
            ...
            if stop_code in found:
                continue
            found[stop_code] = (li.get("data-name") or "普通", code_line, node, li.get("data-date", ""))
    return found
