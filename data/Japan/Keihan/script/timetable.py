...
LINE_MAIN = "00000285"   # 京阪本線（40 站）
LINE_OTO = "00000281"    # 京阪本線(鴨東線直通)（出町柳/神宮丸太町/三条）
# 直通上幹線的連絡線：掃其端點站才抓得到中之島発/交野発/宇治発的直通車。
# 這些線的站不在幹線拓樸內，convert 會自動略過其區段，只留幹線部分。
LINE_NAKANOSHIMA = "00001094"  # 中之島線（中之島→天満橋接入，大量直通出町柳/萱島等）
LINE_KATANO = "00000283"       # 交野線（私市→枚方市接入，尖峰部分直通）
LINE_UJI = "00000280"          # 宇治線（宇治→中書島接入，少數直通）
LINE_KEISHIN = "00000282"      # 京津線（大津線群，獨立網，御陵↔びわ湖浜大津）
LINE_ISHIYAMA = "00000284"     # 石山坂本線（大津線群，石山寺↔坂本比叡山口）
LINE_CABLE = "00001088"        # 鋼索線（男山ケーブル，獨立）
# 掃描來源；本線/鴨東優先（共用站以本線 lineId 為準），再加其餘各線。
SCAN_LINES = [LINE_MAIN, LINE_OTO, LINE_NAKANOSHIMA, LINE_KATANO, LINE_UJI,
              LINE_KEISHIN, LINE_ISHIYAMA, LINE_CABLE]
