...
- **非 JS 的 HTML 不依 URL 的 `date`/`updown` 過濾**：timetable listing 固定回傳「**伺服器當天起約 4 天**」的池子，URL 帶的 `year/month/day` 參數**完全被無視**（實測：帶不同日期回傳完全相同的池子）。每班 `<li.time-frame>` 自帶 `data-date`(該班真正運行日)、`data-name`(種別)、`data-dest`、`data-node`。→ **絕對不可用 URL/href 的日期字串篩營運日**（會抓到錯的日子，甚至平日/假日兩檔都拿到同一天）；要**讀每班 `data-date` 的星期幾分平日/土休**：單次掃描收集整個池子 → 依曜日分桶 → 各取「班次最多的代表日」。⚠️ 4 天窗口**只有在週三〜週日執行時才同時含平日與週末**（週一/二會缺週末）→ 爬蟲應檢查池子是否兩類都有、否則提示換日執行。種別優先取 `data-name` 屬性（比解析連結文字可靠，且平日的通勤特急/通勤急行/快速等尖峰專屬種別才抓得到）。方向由 stops 序列自身判斷（不靠 updown）。
