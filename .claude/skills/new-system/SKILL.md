...
- **非 JS 的 HTML 不依 URL 的 `date`/`updown` 過濾**，而是回傳近一週的池子；每班 `<li.time-frame>` 自帶 `data-date`、`data-name`(種別)、`data-dest`、`data-node`。→ **依 `data-date` 篩營運日**（平日抓週三、土休抓週六），方向由 stops 序列自身判斷（不靠 updown）。
...
- **外運營商直通段（智頭急行式）**：直通車跑進別家公司的區間（如 JR西特急直通京丹後、或反向 JR 跑進智頭急行）。那些站不在本系統拓樸 → **不要丟掉**，收進 `{"id":..., "s":[站名...], "t":[...], "v":[...], "is_other":true, "system_name":"JR西日本", "system_path":"data/Japan/JR_West/"}` segment。前端**不畫在運行圖**，但會在底部資訊面板以該 operator 名標註並列出停靠站（可點 system_path 跳系統）。validator 會對 is_other 的段/站出 WARNING（如同 JR_West 的智頭），屬正常。`s` 用站名字串即可（外站無拓樸 id）。
